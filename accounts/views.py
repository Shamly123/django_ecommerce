...
def register_page(request):
    if request.method == "POST":
        ...
        message = (
            f"hi please enter the below code to complete verification : {email_token}"
        )
...
def user_profile(request):
    counter = 0
    total_orders = Order.objects.filter(status="Order placed").aggregate(
    ...
    recent_orders = Order.objects.filter(
        Q(user=request.user), ~Q(status="Order Pending")
    ).order_by("-created_at")[:2]
    total_wishlist = Wishlist.objects.filter(user=request.user).aggregate(
        total_wishlist=Count("uid")
    )["total_wishlist"]
...
def reset_password(request):
    if "password_reset_email" not in request.session:
        messages.warning(
            request, "Invalid request. Please start the password reset process again."
        )
        return redirect("forgotPassword")
...
def user_single_order(request, order_id):
    try:
        ...
        return render(
            request,
            "user-single-order-details.html",
